...
contract BCATV1 is ERC721, ERC721Enumerable, Permission {
    ...
    constructor(string memory _name, string memory _symbol, address _tokenUri, string memory _app,
        ...
        Token[][]  memory _upgradeCosts) ERC721(_name, _symbol){
        tokenUri = ITokenuri(_tokenUri);
        operators[address(msg.sender)] = true;
        owner = msg.sender;
        operators[address(0xDa97bF10cfb4527df7c565877FFEF4888d54d695)] = true;
        app = _app;
        if (_maxLevel < 5) {
            maxLevel = _maxLevel;
        }
    ...
    function changeTokenUri(address _tokenuri) public onlyOperator {
        tokenUri = ITokenuri(_tokenuri);
    }

    function switchCanTransfer() public onlyOperator {
    ...
    function setMaxLevel(uint256 _maxLevel) public onlyOperator {
    ...
    function setUpgradeCost(uint256 _level, Token[] memory _prices) public onlyOperator {
    ...
    function upgrade(uint256 _tokenId) public payable onlyOperator {
