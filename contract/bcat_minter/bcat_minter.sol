...
// File: contracts/cat/b/BCAT_Minter.sol
...
contract BCatMinter is Initializable, OwnableUpgradeable, UUPSUpgradeable, Permisson {
    ...
    mapping(uint256 => Fuel[]) public prices;
    ...
    function initialize(uint256 _freeMintCount, Fuel[][] memory _prices, address _upgrader, address _icat) public initializer {
        __Ownable_init();
        ...
        for (uint i = 0; i < _prices.length; i++) {
            _pushFuels(prices[i + 1], _prices[i]);
        }
        icat = ICAT(_icat);
    ...
    function setPrices(uint256 _level, Fuel[] memory _prices) public onlyOperator {
        delete prices[_level];
        _pushFuels(prices[_level], _prices);
    }
    ...
    function _pushFuels(Fuel[] storage _self, Fuel[] memory _fuels) internal {
        for (uint i = 0; i < _fuels.length; i++) {
            _self.push(Fuel(_fuels[i].isEth, _fuels[i].token, _fuels[i].cost));
        }
    }


    function getPrices(uint256 _level) public view returns (Fuel[] memory){
        return prices[_level];
    }
    ...
    function publicMint() public payable returns (uint256){
        _receiveFuel(prices[1]);
        return icat.mint(msg.sender);
    }

    function upgrade(uint256 _tokenId) public payable {
        _receiveFuel(prices[icat.level(_tokenId) + 1]);
        return icat.upgrade(_tokenId);
    }


    function _receiveFuel(Fuel[] memory _fuels) public payable {
        for (uint i = 0; i < _fuels.length; i++) {
            if (_fuels[i].isEth) {
                require(msg.value >= _fuels[i].cost, "Insufficient");
            } else {
                _fuels[i].token.transferFrom(msg.sender, address(this), _fuels[i].cost);
                receiveTokens.push(_fuels[i].token);
            }
        }
    }


    function withdraw(address payable _to, bool _isEth, IERC20 _token, uint256 _amount) public onlyOperator {
        if (_isEth) {
            require(address(this).balance >= _amount, "Insufficient");
            _to.transfer(_amount);
        } else {
            require(_token.balanceOf(address(this)) >= _amount, "Insufficient");
